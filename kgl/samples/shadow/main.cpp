...
Framebuffer *fbuf; // framebuffer for final result

// pcss specific
Framebuffer *shadowmap; // depth buffer for shadow map

// esm specific
Framebuffer *expBuffer; // stores the exponential e^cz where c is a large constant and z is the distance to the blocker
Framebuffer *filterBuffer; // extra buffer for filtering expBuffer
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    ShaderProgram esmgen("esmgen.glsl", Shader::VERTEX_SHADER | Shader::FRAGMENT_SHADER);
    ShaderProgram esmshader("esm.glsl", Shader::VERTEX_SHADER | Shader::FRAGMENT_SHADER);
    ...
    expBuffer = new Framebuffer(params);
    params.depthEnable = false;
    filterBuffer = new Framebuffer(params);
    ...
    while (window->isActive()) {
        ...
        if (shadowMode == PCSS_MODE) {
            ...
            // draw from camera's point of view
            {
                ...
                smapRender.bind(0);
                mats.pushMatrix(MatrixStack::MODELVIEW);
                mats.scale(0.4f / window->getAspect(), 0.4f, 1.0f);
                mats.matrixToUniform(MatrixStack::MODELVIEW);
                mats.matrixToUniform(MatrixStack::PROJECTION);
                quad.draw();
                mats.popMatrix(MatrixStack::MODELVIEW);
            ...
            expBuffer->bind();
            {
                ...
                esmgen.use();
                lmats.initUniformLocs(esmgen.getUniformLocation("mvMatrix"), esmgen.getUniformLocation("pjMatrix"));
                lmats.matrixToUniform(MatrixStack::MODELVIEW);
                lmats.matrixToUniform(MatrixStack::PROJECTION);
                gquad.draw();
                testobj.draw(esmgen);
            ...
            // draw from camera's point of view
            {
                ...
                glActiveTexture(GL_TEXTURE0);
                tex.bind(); // white texture for floor

                glActiveTexture(GL_TEXTURE4);
                filterBuffer->bindColorTexture();

                esmshader.use();
                mats.initUniformLocs(esmshader.getUniformLocation("mvMatrix"), esmshader.getUniformLocation("pjMatrix"));
                mats.matrixToUniform(MatrixStack::MODELVIEW);
                mats.matrixToUniform(MatrixStack::PROJECTION);
                lmats.initUniformLocs(esmshader.getUniformLocation("lightmv"), esmshader.getUniformLocation("lightpj"));
                //~ glUniform1f(shadowshader.getUniformLocation("Ka"), 0.2);
                //~ glUniform1f(shadowshader.getUniformLocation("Kd"), 0.8);
                glUniform1i(esmshader.getUniformLocation("map_Ka"), 0);
                glUniform1i(esmshader.getUniformLocation("map_Kd"), 0);
                glUniform1i(esmshader.getUniformLocation("shadowTex"), 4);
                glUniform3fv(esmshader.getUniformLocation("light_Pos"), 1, &light.pos_.x);
                //~ glUniform1f(shadowshader.getUniformLocation("lightNearPlane"), light.near_);
                //~ glUniform1f(shadowshader.getUniformLocation("lightFarPlane"), light.far_);
